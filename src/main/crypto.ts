...
export function resetCryptoCache(){
    cachedPrimary = null; cachedSecondary = null; warned = false;
}
...
async function loadKeys(): Promise<{ primary: CryptoKey | null; secondary: CryptoKey | null; }> {
    ...
    if(primarySecret){
        ...
        cachedPrimary = await crypto.subtle.importKey("raw", mat, { name: "AES-GCM" }, false, ["encrypt","decrypt"]);
    }
    if(previousSecret){
        ...
        if(!primarySecret || previousSecret !== primarySecret){
            ...
            cachedSecondary = await crypto.subtle.importKey("raw", mat2, { name: "AES-GCM" }, false, ["encrypt","decrypt"]);
        }
    }
...
export async function encryptForStorage(value: string | undefined | null): Promise<string | undefined | null> {
    if(value == null || value === "") return value;
    if(value.startsWith(ENC_PREFIX) || value.startsWith(ENC_PREFIX2)) return value; // déjà chiffré
    const { primary } = await loadKeys();
    if(!primary) {
        if(!warned) { console.warn("[crypto] Aucune clé de chiffrement (ENC_SECRET) – les clés API resteront en clair."); warned = true; }
        return value; // fallback clair
    }
    const iv = crypto.getRandomValues(new Uint8Array(12));
    const cipher = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, primary, new TextEncoder().encode(value));
    // Utiliser nouveau format pour tout nouvel encrypt
    return `${ENC_PREFIX2}0:${b64(iv.buffer)}:${b64(cipher)}`;
}

export async function decryptFromStorage(stored: string | null): Promise<{ plain: string | null; reencrypted?: string | null; }>{
    ...
    if(stored.startsWith(ENC_PREFIX)) {
        ...
        try {
            ...
            const plainBuf = await crypto.subtle.decrypt({ name: "AES-GCM", iv }, primary, ct).catch(async err => {
                // Essayer secondary
                if(secondary) {
                    try { return await crypto.subtle.decrypt({ name: "AES-GCM", iv }, secondary, ct); } catch { throw err; }
                }
                throw err;
            });
            ...
            return { plain, reencrypted: reencrypted && reencrypted !== stored ? reencrypted : undefined };
        } catch(e) {
...
export function isEncrypted(value: string | null | undefined) {
    return !!value && (value.startsWith(ENC_PREFIX) || value.startsWith(ENC_PREFIX2));
}
