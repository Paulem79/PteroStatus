...
import { getPing, updatePingNodesFilter, PingRow } from "../sql/requests.ts";
...
function buildNodeButtons(nodes: NodeAttributes[], included: number[], ping: PingRow, guildId: string, page: number) {
  ...
  for (const n of slice) {
    const inside = included.includes(n.id);
    const btn = new ButtonBuilder().setCustomId(`pn|n|${guildId}|${ping.id}|${n.id}`).setLabel(`#${n.id}`).setStyle(inside ? ButtonStyle.Primary : ButtonStyle.Secondary);
    current.push(btn);
  ...
  if (totalPages > 1) {
    ...
    nav.push(new ButtonBuilder().setCustomId(`pn|p|${guildId}|${ping.id}|${Math.max(0, page - 1)}`).setLabel("◀").setStyle(ButtonStyle.Secondary).setDisabled(page === 0));
    nav.push(new ButtonBuilder().setCustomId("pn|noop").setLabel(`Page ${page + 1}/${totalPages}`).setStyle(ButtonStyle.Secondary).setDisabled(true));
    nav.push(new ButtonBuilder().setCustomId(`pn|p|${guildId}|${ping.id}|${Math.min(totalPages - 1, page + 1)}`).setLabel("▶").setStyle(ButtonStyle.Secondary).setDisabled(page === totalPages - 1));
    rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(...nav));
  }
  return rows.slice(0, 5);
}

export async function subNodes(interaction: ChatInputCommandInteraction<"cached">) {
...
export async function handleNodeButton(interaction: ButtonInteraction<"cached">) {
