import { Colors, EmbedBuilder, MessageFlags } from "../../deps.ts";
...
export async function subList(interaction: ChatInputCommandInteraction<"cached">) {
  ...
  const embed = new EmbedBuilder().setColor(Colors.Blue).setTimestamp().setTitle("Pings du serveur");
  ...
  if (pings.length === 0) {
    ...
    for (const ping of pings) {
      ...
      pingBuilder
        .line(`Créé le <t:${Math.floor(ping.created_at.getTime() / 1000)}:F>`) // date
      ...
      if (ping.nodes_filter && ping.nodes_filter.length > 0) {
        ...
        const nodeNames = await Promise.all(filter.map(async nodeId => {
      ...
      } else pingBuilder.line(`Serveurs inclus: Tous`);
