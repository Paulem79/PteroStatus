...
import {getPing, setPingChannel, updatePingCredentials, updatePingHostname, updatePingName} from "../sql/requests.ts";
...
export async function subEdit(interaction: ChatInputCommandInteraction<"cached">) {
    ...
    const newHostName = interaction.options.getString("hostname")?.trim();
    ...
    if (!newHostName && !newAppKey && !newClientKey && !newChannel && !newName) {
    ...
    if (newHostName) {
        const ok = await updatePingHostname(ping.id, guildId, newHostName);
        if (!ok) {
            await interaction.reply({ content: "Échec mise à jour du domaine du panel.", flags: MessageFlags.Ephemeral });
            return;
        }

        messageBuilder.line("🌐 Domaine du panel mis à jour");
        ping.base_url = newHostName;
    }
    ...
    if (newAppKey || newClientKey) {
        ...
        if (newAppKey) {
            messageBuilder.line("🔑 App Key mise à jour");
            ping.app_key = newAppKey;
        }
        if (newClientKey) {
            messageBuilder.line("🗝️ Client Key mise à jour");
            ping.client_key = newClientKey;
        }
    }
