...
export async function startPinger(
  ...
  const ping = pings.find((p) =>
    p.id === id && (guildId ? p.guild_id === guildId : true)
  );
  ...
  if (!messageId) {
    const sent = await channel.send({
      content: `Initialisation du statut (${ping.name})...`,
    });
    messageId = sent.id;
...
export async function triggerPingUpdate(
  client: Client<true>,
  id: number,
  guildId: string,
) {
...
async function updatePingMessage(
  id: number,
  client: Client<true>,
  messageId: string,
  guildId: string,
  ...
  try {
    ...
    const sent = await channel.send({
      content: `Recréation du statut (${ping.name})...`,
    });
  ...
  if (!nodes) {
    embed.setDescription(
      "Impossible de récupérer les nodes (la clé d'API de l'application est-elle correcte ?).",
    );
  ...
  if (ping.nodes_filter) {
    try {
      ...
      console.error(
        "Erreur de parsing des IDs de nodes pour le ping",
        id,
        ping.nodes_filter,
      );
  ...
  for (const raw of nodes.data) {
    ...
    for (const server of servers) {
      ...
      const resources = sid
        ? await getResources(ping.base_url, ping.client_key, sid)
        : undefined;
      ...
      const state = await stateToEmoji(
    ...
    if (serverMessage.length() === 0) {
      serverMessage.line(`En Ligne ${await stateToEmoji("running")}`);
    }
