...
function serverKey(serverMini: ServerMini): string {
  return (serverMini.uuid || serverMini.identifier ||
    (serverMini.id != null ? String(serverMini.id) : "unknown")) as string;
}
...
function buildServersEmbed(
  ...
  const slice = servers.slice(
    page * PAGE_SIZE_SERVERS,
    page * PAGE_SIZE_SERVERS + PAGE_SIZE_SERVERS,
  );
  const embed = new EmbedBuilder()
    .setTitle(`Gestion des serveurs • ${ping.name} • Node #${nodeId}`)
    .setDescription(
      `Cliquez sur un bouton pour inclure / exclure un serveur du filtre. Page ${
        page + 1
      }/${totalPages}`,
    )
    .setTimestamp();
...
function buildServerButtons(
  ...
  for (const server of slice) {
    ...
    if (current.length === 5) {
      rows.push(
        new ActionRowBuilder<ButtonBuilder>().addComponents(...current),
      );
      current = [];
    }
  }
  ...
  if (totalPages > 1) {
    ...
    nav.push(
      new ButtonBuilder().setCustomId(
        `ps|p|${guildId}|${ping.id}|${nodeId}|${Math.max(0, page - 1)}`,
      ).setLabel("◀").setStyle(ButtonStyle.Secondary).setDisabled(page === 0),
    );
    nav.push(
      new ButtonBuilder().setCustomId("ps|noop").setLabel(
        `Page ${page + 1}/${totalPages}`,
      ).setStyle(ButtonStyle.Secondary).setDisabled(true),
    );
    nav.push(
      new ButtonBuilder().setCustomId(
        `ps|p|${guildId}|${ping.id}|${nodeId}|${
          Math.min(totalPages - 1, page + 1)
        }`,
      ).setLabel("▶").setStyle(ButtonStyle.Secondary).setDisabled(
        page === totalPages - 1,
      ),
    );
...
export default new Command({
  ...
  async execute(interaction) {
    ...
    const included = ping.servers_filter
      ? JSON.parse(ping.servers_filter) as string[]
      : [];
    const page = 0;
    const embed = buildServersEmbed(ping, nodeId, servers, included, page);
    const components = buildServerButtons(
      servers,
      included,
      ping,
      guildId,
  ...
  async autocomplete(interaction) {
    ...
    if (focused.name === "nodeid") {
      ...
      const suggestions = entry.nodes
        .filter((n) =>
          valueStr === "" || n.id.toString().includes(valueStr) ||
          n.name.toLowerCase().includes(valueStr)
        )
        .slice(0, 25)
  ...
  async button(interaction) {
    ...
    if (parts[1] === "s") {
      ...
      const servers = extractServers(node);
      let current = ping.servers_filter
        ? JSON.parse(ping.servers_filter) as string[]
        : [];
      if (current.includes(serverKeyToggle)) {
        current = current.filter((k) => k !== serverKeyToggle);
      } else current.push(serverKeyToggle);
