...
function buildNodesEmbed(
  ...
  const slice = nodes.slice(
    page * PAGE_SIZE_NODES,
    page * PAGE_SIZE_NODES + PAGE_SIZE_NODES,
  );
  const embed = new EmbedBuilder()
    .setTitle(`Gestion des nodes • ${ping.name}`)
    .setDescription(
      `Cliquez pour ajouter/enlever un node du filtre. Page ${
        page + 1
      }/${totalPages}`,
    )
    .setTimestamp();
  ...
  if (slice.length === 0) {
    embed.addFields({ name: "Aucun node", value: "Liste vide" });
  }
  return embed;
}
...
function buildNodeButtons(
  ...
  if (totalPages > 1) {
    ...
    nav.push(
      new ButtonBuilder()
        .setCustomId(
          `pn|p|${guildId}|${ping.id}|${Math.min(totalPages - 1, page + 1)}`,
        )
        .setLabel("▶")
        .setStyle(ButtonStyle.Secondary)
...
export default new Command({
  ...
  async execute(interaction) {
    ...
    if (!entry || (now - entry.at) > 30_000) {
      const apiNodes = await getNodes(ping.base_url, ping.app_key) as
        | Nodes
        | undefined;
      if (!apiNodes) {
        await interaction.reply({
          content: "Impossible de récupérer les nodes.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
    ...
    const included = ping.nodes_filter
      ? JSON.parse(ping.nodes_filter) as number[]
      : [];
    const page = 0;
    const embed = buildNodesEmbed(ping, entry.nodes, included, page);
    const components = buildNodeButtons(
      entry.nodes,
      included,
      ping,
      guildId,
  ...
  async button(interaction) {
    ...
    if (parts[1] === "n") {
      ...
      let current = ping.nodes_filter
        ? JSON.parse(ping.nodes_filter) as number[]
        : [];
      if (current.includes(nodeId)) {
        current = current.filter((n) => n !== nodeId);
