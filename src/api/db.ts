...
export async function listPingAutocomplete(interaction: AutocompleteInteraction<"cached">) {
    ...
    if(focused.name === 'id') {
        ...
        const filtered = pings
            ...
            .slice(0,25)
            .map(p=> {
                const channel = p.channel_id ? client.channels.cache.get(p.channel_id) : undefined;
                const ch = channel && channel instanceof GuildChannel ? channel.name : "???"
